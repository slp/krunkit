...
fn get_image_format(disk_image: String) -> Result<DiskImageFormat, io::Error> {
    ...
    if buffer == QCOW_MAGIC {
        log::debug!("Image detected as Qcow2");
        Ok(DiskImageFormat::Qcow2)
    } else {
        log::debug!("Image deteced as Raw");
...
impl TryFrom<Args> for KrunContext {
    ...
    fn try_from(args: Args) -> Result<Self, Self::Error> {
        ...
        if let Some(ref disk_image) = args.disk_image {
            ...
            // If we get the image format wrong, EDK2 will fail to find a kernel
            // and stop there, way before any writes may reach the disk image.
